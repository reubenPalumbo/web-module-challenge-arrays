...
function removeFlavorByName(arr, flavor) {
  ...
  return `Could not find ${flavor}, check spelling.`;
}
...
/// console.log(copy(originalFlavors, coppiedArr));
...
function getAverageWordLength(arr) {
  let avgHold = 0;
  for (let i = 0; i < arr.length; i++) {
    avgHold = +wordCounter(arr[i]);
  }
  avgEnd = avgHold / (arr.length + 1);
  return avgEnd;
}

// console.log(
//   `The average number of words for the array originalFlavors is: ${getAverageWordLength(
//     originalFlavors
//   )}`
// );
...
function getRandomFlavors(arr1, arr2, arr3, arr4) {
  ...
  for (let i = 0; i < arr3.length; i++) {
    combinedArr.push(arr3[i]);
  }
  for (let i = 0; i < arr4.length; i++) {
    combinedArr.push(arr4[i]);
  }

  let tempNum = 0;

  for (let i = 0; i < combinedArr.length && i < 31; i++) {
    tempNum = getRandInt(1, combinedArr.length);
    new31flavors.push(combinedArr[tempNum]);
    combinedArr.splice(tempNum, 1);
  }
